...
export default function ProductCard({
  ...
  const addToHistory = async (e) => {
    ...
    try {
      ...
      const r = await fetch("/api/me/products", {
        ...
        body: JSON.stringify({
          userId: user.id, productUrl, title, imageUrl, store, currency: "CLP", price
        })
      });
      const json = await r.json().catch(() => ({}));
      if (!r.ok) { setErr(json?.error || `Error ${r.status}`); return; }
      setAdded(true);
  ...
  return (
    <a className={`product-card ${className}`} href={productUrl} target="_blank" rel="noreferrer" title={title}>
      ...
      <div className="product-card__body">
        ...
        {pricePerSubUnit ? <div className="product-card__unit">{fmtCLP(pricePerSubUnit)}/un</div> : null}
        {store ? <div className="product-card__store">{store}</div> : null}

        {/* En historial mostramos “Visto por última vez” */}
        {variant === "history" && (
          <small className="product-card__lastSeen">
            Visto por última vez: {lastSeenAt ? new Date(lastSeenAt).toLocaleString("es-CL") : "-"}
          </small>
        )}

        {/* Acciones (columna, full width) */}
        <div className="product-card__actions">
          <button
            type="button"
            className="btn btn-primary"
            onClick={(e) => { e.preventDefault(); e.stopPropagation(); if (productUrl) window.open(productUrl, "_blank", "noopener,noreferrer"); }}
          >
            Ver
          </button>

          {showAddButton && (
            <button type="button" onClick={addToHistory} disabled={loading || added} className="btn btn-dark">
          ...
          {err ? <small className="product-card__msg-error" style={{ color: "crimson", display: "block" }}>{err}</small> : null}
        </div>
      </div>
    </a>
  );
